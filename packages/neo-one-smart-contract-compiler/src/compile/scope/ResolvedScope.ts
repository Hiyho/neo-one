...
export class ResolvedScope implements Scope {
  ...
  private surround(
    ...
    sb.withProgramCounter((loopPC) => {
      ...
      sb.emitOp(node, 'LT');
      // [idx, scope, scopes]
      sb.emitJump(node, 'JMPIFNOT', loopPC.getLast());
      // [scope, idx, scopes]
      sb.emitOp(node, 'SWAP');
      // [scope, scope, idx, scopes]
      sb.emitOp(node, 'DUP');
      // [idx, scope, scope, scopes]
      sb.emitOp(node, 'ROT');
      ...
      sb.emitOp(node, 'INC');
      // [idx, scope, scopes]
      sb.emitJump(node, 'JMP', loopPC.getFirst());
    });
    // [scope, scopes]
    sb.emitOp(node, 'DROP');
    // []
  ...
  private emitNonLocal(
    ...
    if (pc != null) {
      sb.withProgramCounter((innerPC) => {
        ...
        sb.emitJump(node, 'JMPIFNOT', innerPC.getLast());
        sb.emitOp(node, 'DROP');
        sb.emitJump(node, 'JMP', pc);
      });
    }
  }
