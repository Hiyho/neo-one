...
export class WhileStatementCompiler extends NodeCompiler<
  WhileStatement
  > {
  ...
  public visitNode(sb: ScriptBuilder, node: WhileStatement, options: VisitOptions): void {
    sb.withProgramCounter((pc) => {
      ...
      sb.emitJump(node, 'JMPIFNOT', pc.getLast());
      sb.visit(
        node.getStatement(),
        sb.breakPCOptions(
          sb.continuePCOptions(options, pc.getFirst()),
          pc.getLast(),
        ),
      );
      sb.emitJump(node, 'JMP', pc.getFirst());
    });
  }
}
