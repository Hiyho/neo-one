...
export class CapturingScriptBuilder extends BaseScriptBuilder<CapturingScope> implements ScriptBuilder {
  private readonly scopes: CapturingScope[] = [];
  private readonly resolvedScopes: Map<Node, Map<number, ResolvedScope>> = new Map();

  public process(sourceFile: SourceFile, onVisit?: () => void): void {
    super.process(sourceFile, onVisit);
    this.resolveScopes();
  }
