...
export class EqualsEqualsHelper extends Helper<Node> {
  ...
  public equalsEqualsType(
    ...
    if (typeUtils.isSame(leftType, rightType)) {
      ...
      (
        typeUtils.hasNull(leftType) ||
        typeUtils.hasUndefined(leftType)
      ) && (
        typeUtils.isOnlyUndefined(rightType) ||
        typeUtils.isOnlyNull(rightType)
      )
    ) {
      // [left]
      sb.visit(this.left, options);
      ...
      sb.visit(this.left, options);
      // [leftNumber]
      sb.emitHelper(this.left, options, sb.helpers.toNumber);
      // [leftNumber]
  ...
  public equalsEqualsLeftNumberRightBooleanOrString(
    ...
    // [rightNumber, left]
    sb.emitHelper(this.right, options, sb.helpers.toNumber);
    // [rightNumber, left]
  ...
  public equalsEqualsUnknown(
    ...
    sb.emitHelper(node, options, sb.helpers.case(
      ...
      () => {
        // [rightPrim, left]
        sb.emitHelper(node, options, sb.helpers.toPrimitive());
        // [left, rightPrim]
        sb.emitOp(node, 'SWAP');
        // [leftPrim, rightPrim]
        sb.emitHelper(node, options, sb.helpers.toPrimitive());
