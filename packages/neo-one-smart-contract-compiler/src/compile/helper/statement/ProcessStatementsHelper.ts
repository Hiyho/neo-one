import { Node, StatementedNode, TypeGuards } from 'ts-simple-ast';
...
export class ProcessStatementsHelper extends Helper<Node & StatementedNode> {
  ...
  private emitStatements(sb: ScriptBuilder, node: Node & StatementedNode, options: VisitOptions): void {
    ...
    node.getFunctions().forEach((func) => {
      sb.visit(func, sb.plainOptions(options));
    });
    ...
    if (compilerStatements == null) {
      ...
      statements = compilerStatements.map((statement: any) => (node as any).getNodeFromCompilerNode(statement));
    }
    statements.forEach((statement: any) => {
      if (!TypeGuards.isFunctionDeclaration(statement)) {
