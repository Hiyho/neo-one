import { BinaryExpression, Node, Expression, SyntaxKind, ts } from 'ts-simple-ast';
...
export default class BinaryExpressionCompiler extends NodeCompiler<
  ...
  private visitExpressionOperatorBase(
    ...
    const visit = (leftHelper: Helper<Node>, rightHelper?: Helper<Node>) => {
      sb.visit(left, options);
      sb.emitHelper(left, options, leftHelper);
      sb.visit(right, options);
      sb.emitHelper(right, options, rightHelper || leftHelper);
    };
    ...
    switch (kind) {
      ...
      case SyntaxKind.PlusToken:
        if (isBinaryNumeric) {
          ...
          sb.emitOp(node, 'TUCK');
          // [isString, value1, value0, value1]
          sb.emitHelper(node, options, sb.helpers.isString);
          // [value1, isString, value0, value1]
          sb.emitOp(node, 'SWAP');
          // [isString, isString, value0, value1]
          sb.emitHelper(node, options, sb.helpers.isString);
          sb.emitHelper(node, options, sb.helpers.if({
            condition: () => {
              // [isEitherString, value0, value1]
            ...
            whenFalse: () => {
              // [number0, value1]
              sb.emitHelper(node, options, sb.helpers.toNumber);
              // [value1, number0]
              sb.emitOp(node, 'SWAP');
              // [number1, number0]
              sb.emitHelper(node, options, sb.helpers.toNumber);
      ...
      case SyntaxKind.AmpersandAmpersandToken: {
        sb.emitHelper(node, options, sb.helpers.if({
          condition: () => {
            ...
            // [leftBoolean, ?left]
            sb.emitHelper(left, sb.pushValueOptions(options), sb.helpers.toBoolean);
          },
          whenTrue: () => {
            if (options.pushValue) {
              sb.emitOp(node, 'DROP');
